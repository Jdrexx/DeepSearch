...
class TorSearchEngine:
    ...
    def check_tor_connection(self):
        try:
            ...
            console.print(
                f"[bold red]Connection Error:[/bold red] Could not connect to Tor. Is Tor running on port 9050?\n{e}"
            )
            return False
    ...
    def search_onion_ahmia(self, query):
        ...
        try:
            response = self.session.get(
                "https://ahmia.fi/search/", params={"q": query}, timeout=20
            )
    ...
    def run_cli(self):
        ...
        if not self.check_tor_connection():
            console.print(
                "[red]ERROR: Tor is NOT active or port 9050 is blocked.[/red]"
            )
            console.print(
                "[yellow]Please open the Tor Browser or start the Tor service first.[/yellow]"
            )
            return
        ...
        while True:
            user_input = Prompt.ask(
                "[bold yellow]Onion Search Query (or 'r' to refresh, 'exit' to quit)[/bold yellow]"
            )
            ...
            if not results:
                ...
                for i, res in enumerate(results[:10], 1):
                    ...
                    if i == 1:
                        console.print(
                            f"[dim italic]Deep Extracting first onion site...[/dim italic]"
                        )
                        content = self.extract_onion_content(res["url"])
                        console.print(
                            Panel(
                                content,
                                title="Onion Site Content",
                                border_style="green",
                            )
                        )
